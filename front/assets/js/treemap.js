	...
	d3.json("assets/data/arbolSiembraD3.json", function(root) {
	  ...
	  function display(d) {
		...
		g.append("foreignObject")
			...
			.html(function(d) { return '' +
				...
				' <p> Entre 2007-2012 : ' + d3.round(d.value,2).toLocaleString() + ' Héctareas (ha) sembradas </p>' + 
				' <p> Héctareas (ha) sembradas del total : ' + formatNumber(d.rate); 
				})
	  ...
	  function name(d) {
		return d.parent
			? " "  + d.name + " - Click para alejar"
			: "DESAGREGACIÓN REGIONAL Y/O SISTEMA PRODUCTIVO - Click para ampliar";
	  }
	  
	  function nameSave(d) {
		return d.parent
			? name(d.parent) + " - " + d.name + " - Click para alejar"
			: d.name;
	  }
